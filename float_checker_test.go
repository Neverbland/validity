...
func TestFloatValidateNonzero(t *testing.T) {
	rules := ValidationRules{"Baz": []string{"Float", "nonzero"}}

	if !ValidateStruct(TestStruct{Baz: 1}, rules).IsValid {
		t.Errorf("Float nonzero validator does not pass.")
	}
